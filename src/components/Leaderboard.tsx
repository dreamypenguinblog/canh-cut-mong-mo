...
// Sắc độ riêng cho từng hạng Top 3 — dùng cho viền/overlay gradient dưới ảnh bìa
// (chỉ dùng để tạo phân cấp thị giác của ảnh bìa, KHÔNG dùng cho nút "Đọc ngay" nữa —
// nút đọc giờ dùng chung một màu ACCENT cho cả 3 hạng).
    ...
    <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      {topNovels.length === 0 ? (
        ...
        <div className="space-y-8">
          {/* TOP 3 — bọc thêm khung ngoài, card tinh gọn hơn, ảnh bìa to hơn nhờ tăng max-width */}
          {top3.length > 0 && (
            <div
              className={`rounded-[26px] sm:rounded-[32px] border p-3 sm:p-6 ${
                isDark ? 'bg-[#241D26] border-[#6B5261]' : 'bg-[#FFFBFD] border-[#F5DFE7]'
              }`}
            >
              <div className="grid grid-cols-3 gap-2.5 sm:gap-6 max-w-md sm:max-w-3xl mx-auto">
                {top3.map((novel, idx) => {
                  const rank = idx + 1;
                  ...
                  return (
                    <div
                      key={novel.id}
                      className={`group relative rounded-[16px] sm:rounded-[20px] border transition-all duration-300 overflow-visible hover:-translate-y-1 ${
                        isDark
                    ...
                    >
                      ...
                      {/* Huy hiệu hạng */}
                      <span
                        className="absolute -top-2 -left-2 z-20 min-w-[20px] h-[20px] sm:min-w-[24px] sm:h-[24px] px-1 rounded-full text-[9px] sm:text-[10px] font-bold flex items-center justify-center border-2 border-white dark:border-[#2B222C] shadow-[0_4px_10px_-6px_rgba(229,143,179,0.45)]"
                        style={{ background: tone, color: '#FFFFFF' }}
                      >
                        #{rank}
                      </span>

                      {/* Khung trong – bọc ảnh bìa, kiểu "khung lồng khung" như NovelCard, ảnh to hơn nhờ khung podium rộng hơn */}
                      <div
                        className={`relative mx-1.5 sm:mx-2 mt-3 p-1.5 sm:p-2 rounded-[14px] sm:rounded-[16px] border ${
                          isDark
                            ? 'bg-gradient-to-b from-[#352936] to-[#2B222C] border-[#6B5261]'
                            : 'bg-gradient-to-b from-[#FFFAFD] to-white border-[#F5DFE7]'
                        }`}
                      >
                        {/* Pill nổi "TOP" đè lên viền khung — tracking bình thường để icon nơ 𝜗𝜚 không bị vỡ */}
                        <span
                      ...
                      <div className="p-1.5 sm:p-2.5 space-y-1 sm:space-y-1.5">
                        ...
                        {/* Nút "Đọc ngay" — thu nhỏ lại và dùng chung một màu ACCENT cho cả 3 hạng */}
                        <button
                          onClick={() => openReader(novel.id)}
                          className="w-full min-h-[18px] sm:min-h-[20px] mt-1 rounded-full text-[6px] sm:text-[7px] uppercase tracking-wide font-semibold transition-colors"
          ...
          {/* Phần còn lại — card ngang, khung "lồng khung" của NovelCard cho ảnh bìa, ảnh to hơn một chút */}
          {rest.length > 0 && (
              ...
              {rest.map((novel, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Khung lồng khung — cover to hơn trước một chút, cùng ngôn ngữ NovelCard */}
                    <div
                    ...
                    >
                      <img
                        ...
                        alt={novel.title}
                        onClick={() => openNovelDetail(novel.id)}
                        className="w-14 h-[74px] sm:w-16 sm:h-20 object-cover rounded-lg cursor-pointer"
                      />
